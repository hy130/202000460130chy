...
HashTestBuffer(void)
{
    char HashBuffer[] = "sdu_cst_20220610";

    char Buffer[] = "Cui Haoyu 202000460130";
    int Size = 23;
    //char* Buffer = (char*)malloc(Size);
    // NOTE(casey): Ask Meow for the hash
    static meow_u8 key1[128];
    meow_u128 Hash = MeowHash(MeowDefaultSeed, Size, Buffer);
    meow_u8* key = MeowHash_inv(Size, Buffer, HashBuffer);
    dump_buf((meow_u8*)key, 128);
    for (int i = 0; i < 128; i++)
        key1[i] = key[i];
    meow_u128 Hash1 = MeowHash(key1, Size, Buffer);
