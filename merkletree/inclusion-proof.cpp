...
treenode* searchfor(treenode* node, uint8_t* hash) {//该函数利用递归寻找对应的hash值的节点位置
	treenode* temp;
	treenode* temp1;
	if (node->lc != NULL) {//如果对应的子节点不为NULL，则继续向下
		temp = searchfor(node->lc, hash);//利用递归，对子节点进行巡查
		temp1 = searchfor(node->rc, hash);
		if (temp == NULL) {//对与未发现的返回NULL
			return temp1;
		}
		else {
			return temp;
		...
		for (int i = 0; i < 32; i++) {//找到叶子节点之后对32个值进行比较
			if (node->data[i] != hash[i])
				return NULL;
		}
		return node;
	}
}
bool inclusion_proof(uint8_t* hash) {
...
0x01, 0x23, 0x45, 0x00, 0x00, 0x00, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10
	...
	input1[0] = input1[0] + 1;
	uint8_t* temp = SM3_encrypt(input1);//提前保存一个其中的数
	input1[0] = input1[0] - 1;
	for (int i = 0; i < 8; i++) {
