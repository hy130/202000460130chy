...
//与SBOX有所不同，存取的是四个大表，分别对应的是在加密循环函数中输入的每个8bit
//4次移位的结果
...
u32* encrypt_SIMD(u32 m[4])
{
    //进行SIMD实现多数据查表，假设加密的明文数量足够多
    u32 result[4];
    getkeys();
    __m256i temp[4], data[4], tempmask;
    tempmask = _mm256_set1_epi32(0xFF);

    temp[0] = _mm256_loadu_si256((const __m256i*)m + 0);//存取256bit数据，以此类推
    temp[1] = _mm256_loadu_si256((const __m256i*)m + 1);
    temp[2] = _mm256_loadu_si256((const __m256i*)m + 2);
    temp[3] = _mm256_loadu_si256((const __m256i*)m + 3);
    //取其中的相同对应位置32bit，具有相同的查表
    data[0] = _mm256_unpacklo_epi64(_mm256_unpacklo_epi32(temp[0], temp[1]), _mm256_unpacklo_epi32(temp[2], temp[3]));
    data[1] = _mm256_unpackhi_epi64(_mm256_unpacklo_epi32(temp[0], temp[1]), _mm256_unpacklo_epi32(temp[2], temp[3]));
    data[2] = _mm256_unpacklo_epi64(_mm256_unpackhi_epi32(temp[0], temp[1]), _mm256_unpackhi_epi32(temp[2], temp[3]));
    data[3] = _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(temp[0], temp[1]), _mm256_unpackhi_epi32(temp[2], temp[3]));

    //转化大小端，其数据存放原先为小端存放
    __m256i index =
        _mm256_setr_epi8(3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12, 3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12);
    data[0] = _mm256_shuffle_epi8(data[0], index);
    data[1] = _mm256_shuffle_epi8(data[1], index);
    data[2] = _mm256_shuffle_epi8(data[2], index);
    data[3] = _mm256_shuffle_epi8(data[3], index);

    for (int i = 0; i < 1; i++) {
        __m256i k = _mm256_set1_epi32(rk[i]);//装载rk
        temp[0] = _mm256_xor_si256(_mm256_xor_si256(data[1], data[2]), _mm256_xor_si256(data[3], k));
        //查表，以SIMD实现多道查表操作
        temp[1] = _mm256_xor_si256(data[0], _mm256_i32gather_epi32((const int*)Table0, _mm256_and_si256(temp[0], tempmask), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table1, _mm256_and_si256(temp[0], tempmask), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table2, _mm256_and_si256(temp[0], tempmask), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table3, _mm256_and_si256(temp[0], tempmask), 4));
    ...
    data[0] = _mm256_shuffle_epi8(data[0], index);//恢复原先的大小端
    data[1] = _mm256_shuffle_epi8(data[1], index);
    data[2] = _mm256_shuffle_epi8(data[2], index);
    data[3] = _mm256_shuffle_epi8(data[3], index);
    //恢复原先的分组
    temp[0] = _mm256_unpacklo_epi64(_mm256_unpacklo_epi32(data[0], data[1]), _mm256_unpacklo_epi32(data[2], data[3]));
    temp[1] = _mm256_unpackhi_epi64(_mm256_unpacklo_epi32(data[0], data[1]), _mm256_unpacklo_epi32(data[2], data[3]));
    temp[2] = _mm256_unpacklo_epi64(_mm256_unpackhi_epi32(data[0], data[1]), _mm256_unpackhi_epi32(data[2], data[3]));
    temp[3] = _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(data[0], data[1]), _mm256_unpackhi_epi32(data[2], data[3]));

    _mm256_storeu_si256((__m256i*)result + 0, temp[0]);//将数据加载到指针进行返回
    _mm256_storeu_si256((__m256i*)result + 1, temp[1]);
    _mm256_storeu_si256((__m256i*)result + 2, temp[2]);
    _mm256_storeu_si256((__m256i*)result + 3, temp[3]);
