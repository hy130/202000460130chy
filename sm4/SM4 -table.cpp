...
u32* encrypt_table(u32 m[4]) {//不再需要Tfunction1
    getkeys();
    for (int i = 0; i < 32; i++) {//32次循环函数
        u32 temp = m[0];
        u32 input = m[1] ^ m[2] ^ m[3] ^ rk[i];
        temp ^= Table0[(input >> 24) & 0xFF];//分别将其中4个8bit进入table中
        temp ^= Table1[(input >> 16) & 0xFF];
        temp ^= Table2[(input >> 8) & 0xFF];
        temp ^= Table3[(input >> 0) & 0xFF];
        for (int i = 0; i < 3; i++) {
            m[i] = m[i + 1];
        }
        m[3] = temp;
